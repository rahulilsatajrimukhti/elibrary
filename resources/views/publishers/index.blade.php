...
@section('content')
    <div class="container-fluid">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            @if (canAccess('publishers.index', 'can_create'))
                <a href="{{ route('publishers.create') }}" class="btn-add-publish">
                    <i class="fas fa-plus"></i>
                    <span class="btn-text">Penerbit</span>
                </a>
            @endif
        </div>
                    ...
                    <tbody>
                        @forelse($data as $key => $row)
                            <tr>
                                ...
                                <td class="small">
                                    {{ Str::limit($row->address, 100, '...') }}
                                    @if (strlen($row->address) > 100)
                                ...
                                <td class="text-center">
                                    ...
                                    @if (canAccess('publishers.index', 'can_edit'))
                                        <a href="{{ route('publishers.edit', $row->id) }}"
                                            class="btn btn-warning btn-sm rounded-circle">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                    @endif

                                    @if (canAccess('publishers.index', 'can_delete'))
                                        <form action="{{ route('publishers.destroy', $row->id) }}" method="POST"
